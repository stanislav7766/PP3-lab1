...
public class Vector {
    ...
    public synchronized Vector filledWithRandomValues() {
        try {
    ...
    public synchronized double findMin() {
        if (!filled)
            throw new Error("Спочатку необхідно викликати filledWithRandomValues");
    ...
    public synchronized Vector multiplyWithDouble(double d) {
        if (!filled)
            throw new Error("Спочатку необхідно викликати filledWithRandomValues");

        double[] resVector = new double[size];
        for (int i = 0; i < size; i++) {
            resVector[i] = kahanSum(resVector[i], _Vector[i] * d);
        }
        _Vector = resVector;
        return this;
    }

    public synchronized Vector sumWithVector(Vector vector) {
        if (!filled)
            throw new Error("Спочатку необхідно викликати filledWithRandomValues");
        ...
        for (int i = 0; i < size; i++) {
            resVector[i] = kahanSum(resVector[i], _Vector[i] + currentVector[i]);
        }
        _Vector = resVector;
        return this;
    }
