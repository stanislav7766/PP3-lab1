...
public class Matrix {
    ...
    public synchronized Matrix filledWithRandomValues() {
        try {
    ...
    public synchronized Matrix multiplyWithDouble(double d) {
        if (!filled)
            throw new Error("Спочатку необхідно викликати filledWithRandomValues");
        ...
        for (int i = 0; i < size; i++) {
            for (int k = 0; k < size; k++) {
                resMatrix[i][k] = 0;
                for (int j = 0; j < size; j++) {
                    resMatrix[i][k] = kahanSum(resMatrix[i][k], _Matrix[i][k] * d);
                }
            }
        }
        _Matrix = resMatrix;
    ...
    public synchronized Matrix sumWithMatrix(Matrix matrix) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                resMatrix[i][j] = kahanSum(resMatrix[i][j], currentMatrix[i][j]);
            }
        }
        _Matrix = resMatrix;
        return this;
